    ...
    <div>
      <div style={{display:'flex', flexDirection:'row', marginLeft:"50px"}}>
        ...
        <div style={{fontSize:"20px", marginLeft:"10px", cursor:"pointer", color:"red"}} onClick={()=>{
          ...
          <ParentFormComponent type ="select" name="tag" defaultValue={state.switchcase}  theme="default" update={true}
        selectOptions={componentList.getList("tag").map((obj, index)=>{return obj.getJson().name})} obj= {state.opps.getUpdater("update")}  />
        </div>
        <RunButton app={app} theme="default"/>
        </>
        )}
...
class TabContent extends Component{
  ...
  async componentDidMount(){
    await this.props.app.state.componentList.sortSelectedList("tag", "order")
  ...
  render(){
    ...
    <div style={{display:"flex", flexDirection:"row", justifyContent:"space-between"}}>
      <div style={{width:window.innerWidth<600?"80%":"90%", display:"flex", flexDirection:"row"}} className="hScroller">
        <div >
      <MapComponent app={app} name="tag" theme="outreachMap" cells={[ "name", {custom: CustomTest, props:{app:app}}]}  functions={{cells:[0,1], functions:[(component)=>{
          app.dispatch({switchcase:component.getJson().name})
      }]}}/></div>
      <div style={{cursor:"poitner", marginLeft:"10px"}} onClick={async ()=>{
        debugger
        let order = parseInt(componentList.getList('tag').length)
        await app.setPopup({operate:"addtag", operation:"cleanJsonPrepare", object:{order:order, _id:Math.floor(Math.random()*10000).toString()}}, "addTag")
      }
        }> + Tab</div>
      </div>
      <div style={{cursor:"poitner",}} onClick={async ()=>{
        await app.setPopup({operate:"addperson", operation:"cleanPrepare", object:1}, "addContact")
      }
        }> + Contact</div>
    </div>
    )
...
class BiggestCardColorTabWhite extends Component{
  ...
  render(){
    ...
    return(
      <div  style={{...styles?.biggestCard}}>   
      <div style={{...styles?.colorTabWhite , borderBottom:"1px solid black", height:"80px"}}> <TabContent app={app} /></div>   
      <div style={{...styles?.biggestCardContentWithTab}} className='scroller'>
        <MainContent app={app} />
        </div>
        </div>
