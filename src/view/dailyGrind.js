...
class MainContent extends Component{
  ...
  async componentDidMount(){
    ...
    let componentList = state.componentList;
    let styles =state.styles;
    let list = componentList.getList('task', true, 'done');
    let updateList = []
    debugger
    for(let item of list){
      if(item.getJson().lastChecked!=="" && item.getJson().lastChecked!==moment().format('L')){
        item.setCompState({done:false});
        updateList.push(item)

      }
    }
    if(updateList.length!==0){
      await state.opps.cleanPrepareRun({update:updateList})
  ...
  render(){
    ...
    <div style={{width:"100%", height:"100%"}}>
      ...
      <div style={{fontSize:"30px"}}>Tasks</div>
      <div style={{cursor:'pointer', marginLeft:"40px"}} onClick={async ()=>{
  
        await app.setPopup({operate:"addtask", operation:"cleanJsonPrepare", object:{ _id:Math.floor(Math.random()*10000).toString()}}, "addTask")
      }}>+ Task</div>
      </div>
      {this.state.start&&(
      <MapComponent app = {app} name="task"theme = "defaultTasks" cells = {["name", {inputType:"checkbox", name:"done", wrapperStyle:{marginLeft:"40px"},  func:(comp, val)=>{
        debugger
        comp[0].markDone()}}, "delete"]} />
      )}
    </div>
    )
  }
